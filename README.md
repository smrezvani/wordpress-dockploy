...
- `../files/99-custom.ini` - PHP settings
...
### Important Notes:
- The `init-configs` service automatically copies `nginx.conf` and `99-custom.ini` from the `config/` directory to the `files/` directory on first deployment
- If you need to update these config files later, you can either:
  - Update them directly in Dokploy's file manager (`files/nginx.conf` and `files/99-custom.ini`)
